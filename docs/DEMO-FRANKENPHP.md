...
There are demos for **Symfony 7** and **8** (**demo/symfony7**, **demo/symfony8**). Each has its own Dockerfile, docker-compose.yml and Makefile. From the bundle root you run e.g. `make -C demo/symfony8 up` (see the demo's README for the URL and port).
...
**Ports:** Each demo uses `PORT` from its `.env`. To run both at once, use different ports (default **8007** for symfony7, **8008** for symfony8).
